...
public class WeatherControl {
    ...
    @GetMapping(value = "/citiesGreaterThan/{temp}", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Weather> getCitiesGreaterThan(@PathVariable Integer temp) {
        Flux<Weather> data = weathers.getCitiesGreaterThan(temp);
        Flux<Long> delay = Flux.interval(Duration.ofSeconds(3));
        return Flux.zip(data, delay).map(Tuple2::getT1);
    }
}